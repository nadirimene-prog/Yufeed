...
const statusOptions: { value: ObligationStatus; label: string; icon: React.ReactNode; color: string }[] = [
    {
        value: "approved",
        label: "Approve",
...
export default function ObligationApprovalModal({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : "Failed to update obligation";
            ...
            <DialogContent className="max-w-2xl bg-[#0f0f1a] border-white/10 text-white">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-white/60">
                        {obligation.obligation_id} - {obligation.article_ref || "No article reference"}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-6 py-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-white/70">Decision</label>
                    ...
                    {/* Note */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-white/70">Note (optional)</label>
                        <textarea
                            value={note}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="max-h-32 overflow-y-auto rounded-lg bg-white/5 border border-white/10 p-2 space-y-1">
                            {loadingData ? (
                                ...
                                <p className="text-sm text-white/40 text-center py-2">No risk entries available</p>
                            ) : (
                                    ...
                                    >
                                        ...
                                        <span
                                            className={cn(
                                                ...
                                                risk.inherent_risk_level === "critical" && "bg-red-500/20 text-red-400",
                                                risk.inherent_risk_level === "high" && "bg-orange-500/20 text-orange-400",
                                                risk.inherent_risk_level === "medium" && "bg-yellow-500/20 text-yellow-400",
                                                risk.inherent_risk_level === "low" && "bg-green-500/20 text-green-400"
                                            )}
                                        >
                ...
                <DialogFooter className="gap-2">
                    ...
                    <Button
                        ...
                        className={cn(
                            "transition-colors",
                            selectedStatus === "approved" && "bg-emerald-600 hover:bg-emerald-700",
                            selectedStatus === "rejected" && "bg-red-600 hover:bg-red-700",
                            selectedStatus === "in_review" && "bg-amber-600 hover:bg-amber-700"
                        )}
                    >
                        {isSubmitting ? (
                            <Loader2 className="h-4 w-4 animate-spin mr-2" />
                        ) : null}
                        {selectedStatus === "approved" ? "Approve Obligation" :
                            selectedStatus === "rejected" ? "Reject Obligation" : "Send to Review"}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
