...
export const analyzeDocument = async (celex: string, force: boolean = false) => {
    const response = await apiClient.post(`/api/compliance/documents/${celex}/analyze`, { force });
    return response.data;
};

export const getAnnotations = async (celex: string): Promise<Annotation[]> => {
    const response = await apiClient.get<Annotation[]>(`/api/compliance/documents/${celex}/annotations`);
    return response.data;
};

export const createAnnotation = async (celex: string, data: AnnotationCreate): Promise<Annotation> => {
    const response = await apiClient.post<Annotation>(`/api/compliance/documents/${celex}/annotations`, data);
    return response.data;
};

export const deleteAnnotation = async (annotationId: number) => {
    const response = await apiClient.delete(`/api/compliance/annotations/${annotationId}`);
    return response.data;
};

export const getComplianceMetrics = async (): Promise<ComplianceMetrics> => {
    const response = await apiClient.get<ComplianceMetrics>('/api/compliance/dashboard/metrics');
    return response.data;
};

export const getHighRiskDocuments = async (limit: number = 10) => {
    const response = await apiClient.get(`/api/compliance/documents/high-risk?limit=${limit}`);
    return response.data;
};

export const getUpcomingDeadlines = async (days: number = 90) => {
    const response = await apiClient.get(`/api/compliance/documents/deadlines?days=${days}`);
    return response.data;
};
...
export const getPolicies = async (params?: {
    ...
    const response = await apiClient.get<PaginatedResponse<Policy>>('/api/policies', { params });
    return response.data;
};
...
export const updatePolicy = async (id: number, data: PolicyUpdate): Promise<Policy> => {
...
export const approvePolicy = async (id: number, note?: string): Promise<Policy> => {
    const response = await apiClient.post<Policy>(`/api/policies/${id}/approve`, { note });
    return response.data;
};

export const getPolicyObligations = async (id: number, params?: {
    skip?: number;
    limit?: number;
...
// Policy Sections
export const getPolicySections = async (policyId: number): Promise<{ items: PolicySection[] }> => {
    const response = await apiClient.get<{ items: PolicySection[] }>(`/api/policies/${policyId}/sections`);
    return response.data;
};

export const createPolicySection = async (policyId: number, data: Partial<PolicySection>): Promise<PolicySection> => {
    const response = await apiClient.post<PolicySection>(`/api/policies/${policyId}/sections`, data);
    return response.data;
};

export const updatePolicySection = async (sectionId: number, data: Partial<PolicySection>): Promise<PolicySection> => {
    const response = await apiClient.patch<PolicySection>(`/api/policies/sections/${sectionId}`, data);
    return response.data;
};
...
export const getRiskCategories = async (params?: {
    ...
    const response = await apiClient.get<{ items: RiskCategory[] }>('/api/risk/categories', { params });
    return response.data;
};

export const getRiskCategoryTree = async (): Promise<{ items: RiskCategoryTree[] }> => {
    const response = await apiClient.get<{ items: RiskCategoryTree[] }>('/api/risk/categories/tree');
    return response.data;
};

export const createRiskCategory = async (data: Partial<RiskCategory>): Promise<RiskCategory> => {
    const response = await apiClient.post<RiskCategory>('/api/risk/categories', data);
    return response.data;
};

export const getRiskCategory = async (id: number): Promise<RiskCategory> => {
    const response = await apiClient.get<RiskCategory>(`/api/risk/categories/${id}`);
    return response.data;
};

export const updateRiskCategory = async (id: number, data: Partial<RiskCategory>): Promise<RiskCategory> => {
    const response = await apiClient.patch<RiskCategory>(`/api/risk/categories/${id}`, data);
    return response.data;
};
...
export const getRiskEntries = async (params?: {
    ...
    const response = await apiClient.get<PaginatedResponse<RiskEntry>>('/api/risk/entries', { params });
    return response.data;
};

export const createRiskEntry = async (data: RiskEntryCreate): Promise<RiskEntry> => {
...
export const updateRiskEntry = async (id: number, data: RiskEntryUpdate): Promise<RiskEntry> => {
    const response = await apiClient.patch<RiskEntry>(`/api/risk/entries/${id}`, data);
    return response.data;
};
...
export const getRiskEntryObligations = async (entryId: number): Promise<{ items: ObligationRiskLink[] }> => {
    const response = await apiClient.get<{ items: ObligationRiskLink[] }>(`/api/risk/entries/${entryId}/obligations`);
    return response.data;
};
...
export const linkObligationToRisk = async (
    ...
    const response = await apiClient.post<ObligationRiskLink>(`/api/risk/entries/${entryId}/obligations`, data);
    return response.data;
};

export const deleteObligationRiskLink = async (linkId: number): Promise<void> => {
...
export const updateObligationStatus = async (id: number, data: { status: string; note?: string }): Promise<Obligation> => {
    const response = await apiClient.patch<Obligation>(`/api/obligations/${id}`, data);
    return response.data;
};

export const approveObligation = async (id: number, data: ObligationApprovalData): Promise<Obligation> => {
    const response = await apiClient.patch<Obligation>(`/api/obligations/${id}/approve`, data);
    return response.data;
};

export const getObligationRisks = async (id: number): Promise<{ items: ObligationRiskLink[] }> => {
    const response = await apiClient.get<{ items: ObligationRiskLink[] }>(`/api/obligations/${id}/risks`);
    return response.data;
};
