...
export default function CaseDetailPage() {
  ...
  const [caseDetails, setCaseDetails] = useState<ComplianceProfile | null>(
    null,
  );
    ...
    const fetchCase = async () => {
      try {
        ...
        const res = await fetchWithAuth(
          `${apiUrl}/compliance/cases/${params.id}`,
        );
        if (res.ok) {
  ...
  const handleReview = async (action: "approve" | "reject") => {
    ...
    try {
      const apiUrl = getApiBaseUrl();
      const res = await fetchWithAuth(
        `${apiUrl}/compliance/cases/${params.id}/review`,
        {
          method: "POST",
              ...
              {caseDetails.type === "kyc" ? (
                <>
                  ...
                  <div className="col-span-2">
                    ...
                    <span className="font-medium">
                      {caseDetails.address_line1}, {caseDetails.city},{" "}
                      {caseDetails.country}
                    </span>
                  </div>
                </>
            ...
            {caseDetails.risk_signals.length === 0 ? (
              ...
              <div className="space-y-3">
                {caseDetails.risk_signals.map((signal, i) => (
                  ...
                  >
                    <div
                      className={`mt-0.5 h-2 w-2 rounded-full mr-3 ${signal.score > 50 ? "bg-red-500" : "bg-yellow-500"}`}
                    ></div>
                    <div>
