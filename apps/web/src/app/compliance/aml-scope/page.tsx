  ...
  return (
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">AML Scope Coverage</h1>
          <p className="text-sm text-gray-500">
            Regulatory obligations mapped to internal controls and policies.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <label className="text-xs uppercase tracking-wide text-gray-500">Jurisdiction</label>
          <select
            value={jurisdiction}
        ...
        <>
          {isEmpty && (
            <div className="rounded-lg border border-dashed border-gray-200 bg-white p-5 text-sm text-gray-600 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-gray-300">
              ...
              <p className="mt-2 text-xs text-gray-500">
                Run ingestion or backfill obligations from existing documents to populate coverage metrics.
              </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="text-xs uppercase text-gray-500">Total obligations</div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white mt-2">{total}</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="text-xs uppercase text-gray-500">Coverage %</div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white mt-2">{coveragePct}%</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="text-xs uppercase text-gray-500">Policy mapped %</div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white mt-2">{policyPct}%</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="text-xs uppercase text-gray-500">Coverage gaps</div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white mt-2">{gaps}</div>
            </div>
          </div>

          <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="text-sm font-semibold text-gray-900 dark:text-white">Jurisdiction coverage</div>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                  {data?.by_jurisdiction?.length ? (
                    data.by_jurisdiction.map((row) => (
                      <tr key={row.jurisdiction} className="text-gray-700 dark:text-gray-300">
                        <td className="py-3 pr-4 font-medium">{row.jurisdiction.toUpperCase()}</td>
                    ...
                    <tr>
                      <td className="py-3 text-gray-500" colSpan={7}>
                        {isEmpty ? "No obligations yet. Run ingestion or backfill to populate scope." : "No data"}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-gray-900 dark:text-white">Coverage gaps</div>
                <p className="text-xs text-gray-500 mt-1">Obligations without internal controls.</p>
              </div>
              <Link href="/compliance/obligations" className="text-xs font-semibold text-indigo-600 hover:text-indigo-500">
                Review obligations
              </Link>
            </div>
            <div className="mt-4 space-y-3">
              {data?.gap_items?.length ? (
                data.gap_items.map((item) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="font-semibold">{item.obligation_id}</div>
                      <div className="text-xs text-gray-500">{formatDate(item.updated_at)}</div>
                    </div>
                    <div className="mt-2 text-xs text-gray-500 line-clamp-2">{item.obligation_text}</div>
                ...
                <div className="text-sm text-gray-500">
                  {isEmpty ? "No obligations yet. Ingest documents to see coverage gaps." : "No gaps detected."}
                </div>
              )}
            </div>
          </div>
