...
const ruleExportColumns: ExportColumn<Record<string, unknown>>[] = [
  { key: "rule_id", label: "Rule ID" },
  { key: "name", label: "Name" },
  { key: "category", label: "Category" },
  { key: "severity", label: "Severity" },
  { key: "enabled", label: "Enabled" },
  { key: "alert_count", label: "Alert Count" },
  { key: "true_positive_rate", label: "True Positive Rate (%)" },
];
...
export default function RuleManagementPage() {
  ...
  const fetchOverview = async () => {
    try {
      const res = await fetchWithAuth(
        `${API_URL}/api/monitoring-rules/statistics/overview`,
      );
  ...
  const fetchPendingVersions = async () => {
    try {
      const res = await fetchWithAuth(
        `${API_URL}/api/monitoring-rules/versions?status=pending`,
      );
      if (!res.ok) throw new Error(await res.text());
  ...
  const handleToggle = async (rule: Rule) => {
    ...
    try {
      ...
      const res = await fetchWithAuth(
        `${API_URL}/api/monitoring-rules/${rule.rule_id}/${endpoint}`,
        {
          method: "POST",
        },
      );
  ...
  const handleDelete = async (rule: Rule) => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_URL}/api/monitoring-rules/${rule.rule_id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleSubmitForApproval = async () => {
    ...
    try {
      const conditions = editConditions.trim()
        ? JSON.parse(editConditions)
        : undefined;
      const thresholds = editThresholds.trim()
        ? JSON.parse(editThresholds)
        : undefined;
      ...
      const res = await fetchWithAuth(
        `${API_URL}/api/monitoring-rules/${editingRule.rule_id}/versions`,
        {
          method: "POST",
  ...
  const handleApproveVersion = async (versionId: number) => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_URL}/api/monitoring-rules/versions/${versionId}/approve`,
        {
          method: "POST",
        },
      );
  ...
  const handleRejectVersion = async (versionId: number) => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_URL}/api/monitoring-rules/versions/${versionId}/reject`,
        {
          method: "POST",
        },
      );
  ...
  const sortedRules = [...rules].sort(
    (a, b) => (b.alert_count || 0) - (a.alert_count || 0),
  );
  ...
  const filteredRules = useMemo(() => {
    ...
    return sortedRules.filter((rule) => {
      ...
      const matchesCategory =
        categoryFilter === "all" ||
        (rule.category || "uncategorized") === categoryFilter;
      return (
        matchesSearch && matchesSeverity && matchesEnabled && matchesCategory
      );
    });
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <ExportButton
              ...
              filename="monitoring-rules"
              pdfTitle="Monitoring Rules Report"
              columns={ruleExportColumns}
              loading={isLoading}
            />
            <a
