...
export default function ModelRegistryPage() {
  ...
  const [trainingWindow, setTrainingWindow] = useState(
    "2025-01-01 to 2025-12-31",
  );
  const [metricsJson, setMetricsJson] = useState(
    '{"auc":0.82,"precision":0.6}',
  );
  ...
  const loadVersions = async (model: Model) => {
    try {
      const res = await fetchWithAuth(
        `${API_URL}/api/models/${model.model_id}/versions`,
      );
  ...
  const handleCreateVersion = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_URL}/api/models/${selectedModel.model_id}/versions`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
