...
def upgrade():
    ...
    if "linked_policy_id" not in existing_columns:
        op.add_column(
            ...
            sa.Column("linked_policy_id", sa.Integer(), sa.ForeignKey("policy_documents.id"), nullable=True)
        )
        op.create_index("ix_regulatory_obligations_linked_policy_id", "regulatory_obligations", ["linked_policy_id"])
    ...
    op.create_index("ix_risk_categories_category_id", "risk_categories", ["category_id"], unique=True)
    ...
    op.create_table(
        ...
        sa.Column("name", sa.String(length=255), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("inherent_risk_level", sa.String(length=50), nullable=False, server_default="medium"),
        sa.Column("residual_risk_level", sa.String(length=50), nullable=False, server_default="medium"),
        sa.Column("likelihood", sa.String(length=50), nullable=True),
        sa.Column("impact", sa.String(length=50), nullable=True),
        sa.Column("mitigation_status", sa.String(length=50), nullable=False, server_default="not_started"),
    ...
    op.create_index("ix_obligation_risk_links_obligation_id", "obligation_risk_links", ["obligation_id"])
    op.create_index("ix_obligation_risk_links_risk_entry_id", "obligation_risk_links", ["risk_entry_id"])
