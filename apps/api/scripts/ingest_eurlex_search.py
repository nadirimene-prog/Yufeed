...
def main() -> None:
    ...
    parser.add_argument(
        "--languages",
        default=settings.EURLEX_LANGUAGES,
        help="Comma-separated languages (default: EURLEX_LANGUAGES)",
    )
    parser.add_argument(
        "--limit", type=int, default=settings.EURLEX_SEARCH_PAGE_SIZE, help="Page size per request"
    )
    ...
    parser.add_argument(
        "--no-lang-filter", action="store_true", help="Disable language filter in SPARQL (fallback)"
    )
    parser.add_argument(
        "--terms-fr", default=settings.EURLEX_SEARCH_TERMS_FR, help="Semicolon-separated FR terms"
    )
    parser.add_argument(
        "--terms-en", default=settings.EURLEX_SEARCH_TERMS_EN, help="Semicolon-separated EN terms"
    )
        ...
        for language in languages:
            ...
            for page in range(args.max_pages):
                ...
                if not results and not args.no_lang_filter:
                    print(
                        f"ℹ️ No results with language filter for {language.upper()} page {page+1}; retrying without language filter."
                    )
                    results = fetcher.search_terms(
                        terms=terms,
                        language=language,
