...
from src.schemas.transaction_schemas import (
    AlertStatistics, TransactionStatistics, MonitoringDashboard,
    AlertResponse, UserRiskProfileResponse, CaseResponse, MonitoringRuleResponse
)
...
def get_monitoring_dashboard(
    ...
    top_risk_users = db.query(UserRiskProfile).filter(
    ...
    rule_performance = db.query(MonitoringRule).filter(
...
def get_realtime_metrics(db: Session = Depends(get_db)):
    ...
    # Alerts in last hour
    alert_count_1h = db.query(func.count(Alert.id)).filter(
        Alert.created_at >= one_hour_ago
    ).scalar() or 0

    # Critical alerts pending
    critical_pending = db.query(func.count(Alert.id)).filter(
        and_(
    ...
    high_risk_1h = db.query(func.count(Transaction.id)).filter(
        and_(
            Transaction.timestamp >= one_hour_ago,
...
def get_system_health(db: Session = Depends(get_db)):
    ...
    unprocessed_txns = db.query(func.count(Transaction.id)).filter(
...
def _get_health_recommendations(stuck_alerts: int, unprocessed_txns: int) -> List[str]:
    ...
    if stuck_alerts > 100:
        recommendations.append("CRITICAL: More than 100 alerts stuck in pending status. Increase analyst capacity or review alert triage process.")
    elif stuck_alerts > 50:
        recommendations.append("WARNING: High number of pending alerts. Consider reviewing alert prioritization.")
    elif stuck_alerts > 0:
        recommendations.append(f"{stuck_alerts} alerts pending for >24 hours. Review and assign to analysts.")

    if unprocessed_txns > 1000:
        recommendations.append("CRITICAL: Large transaction processing backlog. Check background worker status.")
    elif unprocessed_txns > 100:
        recommendations.append("WARNING: Transaction processing delay detected. Monitor worker performance.")
