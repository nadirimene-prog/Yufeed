...
CELLAR_CACHE_HITS = Counter(
    'ingestion_cellar_cache_hits_total',
    'CELLAR cache hit count'
)

CELLAR_CACHE_MISSES = Counter(
    'ingestion_cellar_cache_misses_total',
    'CELLAR cache miss count'
)
...
AI_ANALYSIS_COST = Counter(
    'ingestion_ai_analysis_cost_dollars',
    'Cumulative AI analysis cost in dollars'
)
...
RAG_CHUNKS_CREATED = Counter(
    'ingestion_rag_chunks_created_total',
...
RELATIONS_STORED = Counter(
    'ingestion_relations_stored_total',
    'Total document relations stored',
    ['relation_type']
)
...
def record_cellar_query(query_type: str, status: str, duration: float = None, cache_hit: bool = False):
...
def record_ai_analysis(status: str, duration: float = None, cost: float = None, input_tokens: int = None, output_tokens: int = None):
