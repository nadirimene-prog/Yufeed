...
class NetworkAnalyzer:
    ...
    def analyze_user_network(self, user_id: str, depth: int = 2, days: int = 90) -> Dict[str, Any]:
        """
        ...
        network_risk = self._calculate_network_risk(
            network, circular_flows, shared_attributes, suspicious_clusters
        )
    ...
    def _build_network(self, user_id: str, depth: int, start_date: datetime) -> Dict[str, Any]:
        """
        ...
        node_details = {}
        for node in nodes:
            profile = self.db.query(UserRiskProfile).filter(UserRiskProfile.user_id == node).first()

            node_details[node] = {
                "user_id": node,
                "risk_level": profile.risk_level if profile else "unknown",
                "risk_score": (
                    float(profile.overall_risk_score)
                    if profile and profile.overall_risk_score
                    else 0
                ),
                "total_alerts": profile.total_alerts if profile else 0,
            }

        return {"nodes": list(nodes), "node_details": node_details, "edges": edges, "depth": depth}
    ...
    def _find_cycles_from_node(
        self, graph: Dict, current: str, visited: Set[str], path: List[Dict], max_depth: int = 5
    ) -> List[List[Dict]]:
        ...
        if current in visited and len(path) > 2:
            # Found a cycle
            cycle_start = next(
                (i for i, step in enumerate(path) if step.get("node") == current), None
            )
        ...
        for neighbor in graph.get(current, []):
            ...
            cycles.extend(
                self._find_cycles_from_node(
                    graph, neighbor["target"], visited.copy(), new_path, max_depth
                )
            )
    ...
    def _find_shared_attributes(self, network: Dict) -> Dict[str, List]:
        ...
        transactions = self.db.query(Transaction).filter(Transaction.user_id.in_(nodes)).all()
    ...
    def _identify_suspicious_clusters(self, network: Dict) -> List[Dict]:
        ...
        highly_connected = [node for node, count in connection_count.items() if count >= 3]
    ...
    def _calculate_network_risk(
        self, network: Dict, circular_flows: List, shared_attributes: Dict, clusters: List
    ) -> float:
        """
        ...
        high_risk_nodes = sum(
            1
            for details in node_details.values()
    ...
    def find_related_users(self, user_id: str, relationship_type: str = "all") -> List[str]:
        """
        ...
        if relationship_type in ["ip", "all"]:
            ...
            if user_ips:
                ip_matches = (
                    self.db.query(Transaction).filter(Transaction.ip_address.in_(user_ips)).all()
                )
        ...
        if relationship_type in ["device", "all"]:
            ...
            if user_devices:
                device_matches = (
                    self.db.query(Transaction)
                    .filter(Transaction.device_fingerprint.in_(user_devices))
                    .all()
                )
