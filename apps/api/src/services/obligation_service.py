...
def mark_related_obligations_for_review(
    ...
    relations = db.query(LegalRelation).filter(
    ...
    for relation in relations:
        ...
        related_doc = db.query(LegalDocument).filter(
            LegalDocument.celex == relation.to_celex
        ).first()
        ...
        related_obligations = db.query(RegulatoryObligation).filter(
    ...
    if marked > 0:
        db.commit()
        logger.info(f"Marked {marked} obligations for review due to document relations from {doc.celex}")
...
def get_obligations_needing_review_due_to_relations(
    ...
    relations = db.query(LegalRelation).filter(
        ...
        LegalRelation.relation_type.in_(["amends", "repeals", "supersedes", "corrects", "replaces"]),
    ).all()
