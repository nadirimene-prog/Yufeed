...
def test_tenant_lifecycle_and_api_keys(db_session):
    ...
    tenant = tenants_api.create_tenant(tenant_payload, db_session)
    assert tenant.tenant_id == "acme_test"

    tenants = tenants_api.list_tenants(skip=0, limit=100, db=db_session)
    assert any(t.tenant_id == "acme_test" for t in tenants)

    fetched = tenants_api.get_tenant("acme_test", db_session)
    ...
    updated = tenants_api.update_tenant_config("acme_test", config_update, db_session)
    assert updated.settings["timezone"] == "UTC"

    stats = tenants_api.get_tenant_stats("acme_test", db_session)
    ...
    keys = tenants_api.list_api_keys("acme_test", include_revoked=False, db=db_session)
    ...
    rotated = tenants_api.rotate_api_key("acme_test", key_id, db_session)
    ...
    tenants_api.revoke_api_key("acme_test", new_key_id, db_session)
    ...
    users = tenants_api.list_tenant_users("acme_test", is_active=None, db=db_session)
    assert any(u.user_id == "user_1" for u in users)

    tenants_api.remove_user_from_tenant("acme_test", "user_1", db_session)
    remaining = db_session.query(TenantUser).filter(TenantUser.user_id == "user_1").first()
    assert remaining is None

    tenants_api.delete_tenant("acme_test", hard_delete=False, db=db_session)
